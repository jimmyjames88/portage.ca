...
(function ($) {
	...
	// form handlers
	// sender
	// $('#sender-form form, #porter-form form').on('submit', function(e){
	// 	e.preventDefault();
	// 	var self = $(this);
	// 	$.post({
	// 		url: '/php/portage-form.php',
	// 		data: $(this).serialize(),
	// 		dataType: 'JSON',
	// 		success: function(response){
	// 			if(response.status == 'success') self.html('<div class="alert alert-success text-center">Thanks! We have received your request and will respond shortly!</div>');
	// 			else self.html('<div class="alert alert-danger text-center">Oops! There was an error processing your request. Please try again later.</div>');
	// 		}
	// 	});
	// });

	// terms of service modal
	$('a.tos').on('click', function(){
		$('#terms-of-service').modal({
		})
	});

	// Form Steps
	$('#sender-form, #porter-form').on('submit', 'form', function(e){
		e.preventDefault();
		return false;
	})
	// Show first form step
	var formWrapper = $('#sender-form, #porter-form');

	// disable enter key
	$('#sender-form form, #porter-form form').on('keypress', 'input', function(e){
		if(e.keyCode == 13){
			e.preventDefault();
			return false;
		}
	...
	formWrapper.each(function(){
		...
		// add keys to forms/steps
		forms.each(function(k, v){
			$(this).attr('data-step', k);
		});

		var fw = $(this); 	//formwrapper reference for scrolling later

		forms.on('click', 'button.next-step', function(){

			var form = $(this).closest('form');
			if(form[0].checkValidity() === true){
				form.fadeOut();
				if(form.next('form').length){
					form.next('form').fadeIn();
					$('html, body').animate({
						scrollTop: fw.find('form:visible').offset().top - 200
					}, 600);
				} else {
					// submit forms
					$.ajax({
						...
						complete: function(response){
							...
							$('html, body').animate({
						        scrollTop: fw.offset().top - 200
						    }, 600);

							if(response.responseJSON.status == 'success' || response.responseText.status == 'success'){
								forms.last().after('<div class="alert alert-success text-center">Your submission has been received, we will be in touch soon!</div>');
							} else {
								forms.last().after('<div class="alert alert-danger text-center">There was an error with your submission, please try again later or email us directly at <a href="mailto:info@portagedelivers.com">info@portagedelivers.com</a></div>');
							}
						}
					});
				}
			} else {
				// showImages html5 validation
				$('input[type=submit]', form).click();
			}
		});

		forms.on('click', 'button.go-back', function(){
			var form = $(this).closest('form');
			form.fadeOut();
			form.prev('form').fadeIn(function(){
				$('html, body').animate({
                ...
                $.ajax({
                    ...
                    complete: function(data){
                        ...
                        if (result.status == 'success') {
                            alert('');
                            document.getElementById('message-info').innerHTML = 'Your message has been received, we\'ll be in touch soon!';
